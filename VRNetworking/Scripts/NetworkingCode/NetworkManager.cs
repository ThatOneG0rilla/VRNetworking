...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    [Header("Logging")]
    public string WebhookUrl = "URL HERE";
    public bool TrackConnected = true;
    public bool TrackJoinedRoom = true;
    public bool TrackLeftRoom = true;
    public bool TrackDisconnected = true;

    private string LastRoom;
    private GameObject TemporaryPlayer;
    private RoomOptions RoomOpts;
    private int CurrentIndex = 0;
    private readonly List<string> DebugLog = new();
    ...
    public override void OnConnectedToMaster()
    {
        ...
        if (TrackConnected) 
            SendMessage($"{PlayFabSettings.staticPlayer.PlayFabId} Connected To Photon Servers.");
    }
    public override void OnJoinedRoom()
    {
        ...
        if (TrackJoinedRoom)
        {
            ...
            StartCoroutine(PostToWebhook($"{PlayFabSettings.staticPlayer.PlayFabId} Joined Room '{LastRoom}'."));
        }
    }
    public override void OnDisconnected(DisconnectCause Cause)
    {
        ...
        if (TrackDisconnected)
            StartCoroutine(PostToWebhook($"{PlayFabSettings.staticPlayer.PlayFabId} Disconnected From Photon Servers."));
}
    ...
    { 
        if (TrackLeftRoom)
            StartCoroutine(PostToWebhook($"{PlayFabSettings.staticPlayer.PlayFabId} Left Room '{LastRoom}'.")); 
    }
    private IEnumerator PostToWebhook(string msg)
    {
        using var request = new UnityWebRequest(WebhookUrl, "POST");
        request.uploadHandler = new UploadHandlerRaw(System.Text.Encoding.UTF8.GetBytes($"{{\"content\":\"{msg}\"}}"));
        request.downloadHandler = new DownloadHandlerBuffer();
        request.SetRequestHeader("Content-Type", "application/json");
        yield return request.SendWebRequest();
        if (request.result != UnityWebRequest.Result.Success) Debug.LogError($"Webhook Error: {request.error}");
    }
    #endregion
